...
# Create a list to store the genre information
genre_info = []

# Create a set to store the processed artists
...
for i in range(0, len(tracks), max_uris_per_request):
    ...
    if response.status_code == 200:
        ...
        for track, track_data in zip(track_batch, tracks_data):
            ...
            random_timestamp = start_date.timestamp() + randint(0, int((end_date - start_date).total_seconds()))
            random_datetime = pd.to_datetime(random_timestamp, unit='s')
            
            last_played = streaming_data.get('last_played', random_datetime)
            ...
            # Check if artist information has already been processed
            if artist_id not in processed_artists:
                ...
                if response.status_code == 200:
                    ...
                    # Append the artist genres to the genre_info list
                    for genre in genres:
                        genre_info.append([uri, genre])
                else:
                    genres = []
                    artist_photo_url = 'https://i.scdn.co/image/ab67616d00001e02ff9ca10b55ce82ae553c8228'

                # Add artist ID to the set of processed artists
...
# Create a DataFrame from the genre information
genre_df = pd.DataFrame(genre_info, columns=['URI', 'Genre'])

# Write the DataFrame to a separate CSV file
genre_df.to_csv('track_genres.csv', index=False)
